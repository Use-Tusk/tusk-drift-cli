...
Sandboxing defaults to `strict` mode when the platform supports it (macOS/Linux). Use `--sandbox-mode auto` to allow a one-time fallback without sandbox on startup failure, or `--sandbox-mode off` to bypass sandboxing entirely. In `strict` mode, replay fails if sandbox setup cannot be initialized/applied.
