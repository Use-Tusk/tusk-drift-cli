...
Create (or update) a CI workflow that runs Tusk Drift replay on every push/PR.

### Goal

Add CI configuration that:
...
1. **Check code hosting/CI type first**
   - Read `state.code_hosting_type` from current state, or briefly explore the codebase.
   - Determine whether this repo uses:
     - GitHub Actions (`.github/workflows/*.yml`)
     - GitLab CI (`.gitlab-ci.yml`)
     - CircleCI (`.circleci/config.yml`)
     - Jenkins (`Jenkinsfile`)
     - another CI system
   - Do NOT skip this phase for non-GitHub CI; still generate/update CI config using CLI commands.

2. **Inspect existing CI config**
   - Look in the appropriate CI config location(s) for relevant patterns.
...
4. **If using GitHub Actions, fetch action docs/contract (authoritative first)**
   ...
   - If NOT using GitHub Actions, skip this step.

5. **Create or update CI configuration**
   ...
   - For newly created CI config files, add a short draft note at the top using the file's native comment syntax:
      - YAML-based CI files (`.yml`/`.yaml`, GitHub/GitLab/CircleCI): use `# ...`
      - Jenkinsfile (Groovy): use `// ...` (or `/* ... */`)
      - If the CI format does not support comments, skip this note.
      - Example for YAML:
            ```yaml
            ...
            ```
   - For GitHub Actions, use this shape for the Drift step:
